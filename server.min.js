...
app.use(compress()),app.get("/",function(e,n){n.sendFile(__dirname+"/client/public/index.html"),n.flush()}),app.use(express["static"]("client/public")),io.sockets.on("connection",function(e){console.log("connected"),users.push(e),io.emit("userCount",users.length),e.on("requestUsers",function(n){var o=checkName(names,n,e.id)
...
i.indexOf(e.challenger)<=-1&&(io.to(o.id).emit("roomResponse",e.challenger),n.join(e.challenger),names=names.filter(function(e){return e.id!==n.id}),io.emit("respondUsers",names))}()}
for(var o in e)n(o)}),e.on("requestArrow",function(){setTimeout(function(){return e.emit("arrowResponse",!0)},2e3)}),e.on("requestWinner",function(e){io.to(e.room).emit("winnerResponse",e.winner)}),e.on("playerPos",function(n){e.broadcast.to(n.room).emit("enemyPos",n.x)}),e.on("playerScore",function(n){e.broadcast.to(n.room).emit("enemyScore",n.score)}),e.on("playerObj",function(n){e.broadcast.to(n.room).emit("enemyObj",{obj:n.obj,type:n.type})}),e.on("leaveGame",function(n){e.leave(n),io.to(n).emit("someoneLeft"),e.emit("leaveResponse")}),e.on("requestRefresh",function(){e.emit("refreshResponse")}),e.on("disconnect",function(){console.log("disconnection")
